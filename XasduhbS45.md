...
- Language: **British English** for prose (e.g., "behaviour", "optimise"). Keep tool output strings as-is.
- Code fences
  - `bash` = commands
...
- **Topic placement**
  - Add new material under the most relevant existing top-level section (`## 0 ...`, `## 1 ...`, etc.).
  - If nothing fits, create a new numbered `##` section and add it to **Contents**.
- **Accuracy**
  - If a course statement is misleading/incorrect, we’ll fix it and briefly call it out under **Notes** (e.g. `Correction:`) so you remember the nuance.
- **Hands-on learning**
  - Prefer adding a few safe, read-only ADB commands; label anything that needs root (`su`) or is device-/OEM-specific.
  - Add 1–3 mini-exercises (short, verifiable, no special tools unless stated).
- When we add a new topic, we’ll use this structure:
  - Goal
  - In plain terms (simplified)
  - Notes
  - ADB commands to try
  - Mini-exercises
...
### Where commands run (common pitfall)
**In plain terms (simplified):** your Windows terminal (PowerShell) cannot run Android binaries like `/system/bin/bootanimation` directly. You must run them **on the device** via `adb shell`.

What *not* to paste into PowerShell (prompt text is not part of the command):
```text
flame:/ # ./system/bin/bootanimation
```

Correct patterns:
```powershell
# One-off (runs on the device, from your host)
adb shell ls -al /system/bin/boot
adb shell /system/bin/bootanimation

# Interactive
adb shell
# now you are on the device:
ls -al /system/bin/boot
/system/bin/bootanimation
exit
```

Notes:
- Prefer **absolute paths** like `/system/bin/bootanimation` instead of `./system/bin/bootanimation`.
- When copying commands, exclude any prompt prefixes (e.g., `flame:/ #`) and any line-number prefixes (e.g., `2|`).
- `bootanimation` is normally started by `init` during boot; running it manually may exit with an error or do nothing. If it fails, check logs:
  - PowerShell: `adb logcat -d | Select-String -Pattern 'bootanim|bootanimation'`
  - Bash: `adb logcat -d | grep -i -E 'bootanim|bootanimation'`
...
### Android Architecture (layers, bottom → top)
...
##### “Property trees” on-device: `/dev/__properties__/`
**Goal:** understand what you’re seeing under `/dev/__properties__`, and why you *can’t* (and shouldn’t) edit it directly.

**In plain terms (simplified):** `/dev/__properties__/` is where Android exposes the shared-memory “property areas” that processes map for fast `getprop` reads. The entries that look like `u:object_r:...:s0` are tied to **SELinux-labelled property types**.

**Notes:**
- Those names look like SELinux contexts because they are. Android uses them as filenames for the different property areas.
- Correction/nuance: it’s basically true that “only init can modify these files”, but the important detail is *how*:
  - you don’t edit `/dev/__properties__/*` yourself
  - processes request changes via `setprop` / `__system_property_set()` → the **property service** in `init` decides whether to apply the update
  - SELinux policy + `*property_contexts` rules determine what your domain can set
- Common copy/paste pitfall:
  - don’t paste the prompt (`flame:/dev/__properties__ #`) as part of your command
  - don’t paste `ls` output lines (like `u:object_r:bootloader_boot_reason_prop:s0`) and expect them to run — they’re filenames, not commands

Example listing (shortened):
```text
/dev/__properties__ # ls
properties_serial
property_info
...
**ADB commands to try:**
```bash
# List the property areas
adb shell ls -al /dev/__properties__

# Show SELinux labels for these entries (if your `ls` supports -Z)
adb shell 'ls -alZ /dev/__properties__ 2>/dev/null || ls -al /dev/__properties__'

# Show only the SELinux-context-named entries
adb shell 'ls -1 /dev/__properties__ | grep -E "^u:object_r:" | head'

# See which property areas your current shell has mapped
adb shell 'cat /proc/$$/maps | grep /dev/__properties__ | head'
```

**Mini-exercises:**
1) Count how many property areas exist on your device:
   - `adb shell 'ls -1 /dev/__properties__ | grep -E "^u:object_r:" | wc -l'`
2) Pick one area (e.g. `log_tag_prop`) and find what property names map to it:
   - `adb shell 'grep -R "log_tag_prop" /system/etc/selinux/*property_contexts | head'`
3) Trigger a controlled SELinux denial and find it in logs:
   - `adb shell 'setprop this.is.not.real 1'`
   - `adb logcat -d | grep -i "avc: denied" | tail`

**Takeaways + follow-ups:**
- `/dev/__properties__/` is a *mechanism* (shared memory), not a config directory to edit.
- Setting a property is a **policy decision** (property service + SELinux), even if reading is just a memory lookup.
...
```bash
grep -R -n "adjasbdiasdbka" /system/etc/selinux/*property_contexts 2>/dev/null
```
...
### Dalvik (historical runtime; replaced by ART)
- Dalvik was Android’s original managed-code runtime (default up to Android 4.4; ART was optional in 4.4 and became the default in Android 5.0+).
- Dalvik executes **DEX** bytecode (typically `classes.dex` inside an APK).
- VM design goal: keep “Java-like” application semantics, but optimise for mobile constraints (RAM, CPU, storage).
- Architecture note: Dalvik bytecode is **register-based** (contrast: classic JVM bytecode is stack-based).
- **JIT (Just-In-Time) compilation** existed in Dalvik: frequently executed code could be compiled at runtime.

#### DEX (Dalvik Executable): Java-like, but compact
**Goal:** understand what a `.dex` file stores and why it’s often smaller than the equivalent set of `.class` files.

**In plain terms (simplified):** a DEX file bundles many classes together and deduplicates common data (strings, types, method/field references) via indexed tables (“pools”). It saves space mainly by reducing repetition rather than “compressing”.

**Notes:**
- A DEX file is *analogous* to Java `.class` files, but it’s a different format:
  - `.class` = typically one file per class (JVM bytecode)
  - `.dex` = many classes per file (DEX bytecode)
- DEX uses indexed tables so code can reference things by small integers:
  - `string_ids`, `type_ids`, `proto_ids`, `field_ids`, `method_ids`, `class_defs`
- Many apps have a single `classes.dex`; large apps can be **multi-dex** (`classes2.dex`, `classes3.dex`, …).
- Connection to APKs: the DEX file is usually stored *inside* the APK ZIP.
  - It might be **Stored** (uncompressed) for faster `mmap()` access (see the APK ZIP section above).

Example: header + key counts (tool output)
```text
Header Magic: dex\n035
Checksum:     381e4f43 (correct)
File Size:    0xb1200   725504
Header Size:  0x70         112
Endian:       0x12345678

String IDs:  8250 strings @0x70
Type   IDs:  1280 types   @0x8158
Field  IDs:  2890 fields  @0xd1ac
Proto  IDs:  1287 protos  @0x9558
Method IDs:  4517 methods @0x12bfc
Class Defs:   831 classes @0x1b924
Data:       585980 bytes @0x22104
```

Example: “magic” bytes in a hex dump (the first 8 bytes are `dex\n035\0`)
```text
00000000  64 65 78 0a 30 33 35 00  ...  |dex.035.|
```

**ADB commands to try (DEX in a real APK):**
```bash
...
# 2) Pull the APK to your host
APK_PATH=$(adb shell pm path $PKG | head -n 1 | sed 's/^package://')
adb pull "$APK_PATH" base.apk

# 3) List DEX files inside (Linux/macOS/WSL)
unzip -l base.apk | grep -E 'classes([0-9]+)?\.dex'

# 4) Extract + inspect the DEX magic
unzip -p base.apk classes.dex > classes.dex
hexdump -C classes.dex | head
```

**PowerShell variant (Windows host):**
```powershell
$pkg = 'com.example.app'
$apkPath = (adb shell pm path $pkg) -replace '^package:',''
adb pull $apkPath base.apk

Expand-Archive -Path .\base.apk -DestinationPath .\apk_out -Force
Get-ChildItem .\apk_out\classes*.dex

# First 8 bytes should be: 64 65 78 0a 30 33 35 00
Format-Hex -Path .\apk_out\classes.dex -Count 8
```

**Mini-exercises:**
1) Pick any installed package, pull its APK, and answer:
   - Is it single-dex or multi-dex?
   - Are the DEX entries stored or deflated? (Linux/macOS/WSL: `unzip -v base.apk | grep classes.dex`)
2) Confirm the DEX version in the magic (`dex\n035\0`, etc.).
3) In one sentence, explain *why* “string/type/method pools” reduce file size without compression.

### ART (Android Runtime)
- Current runtime environment for Android (Android 5.0+)
- **AOT (Ahead-Of-Time) compilation**: code can be compiled ahead-of-time by `dex2oat` (at install time or later via background dexopt), based on profiles and device policy.
...
#### ART compilation artefacts (`oat/`: `base.odex`, `base.vdex`, `base.art`)
**Goal:** understand the files you see under an app’s `oat/<abi>/` directory.

**In plain terms (simplified):** ART still starts from DEX bytecode, but it can pre-compile and pre-optimise parts of an app. Those results are cached on disk so the app starts faster and doesn’t have to redo expensive work (verification/compilation) every launch.

**Notes:**
- Location (device/version-dependent, and often permission-gated):
  - `/data/app/<random>/<package>-<random>/oat/<abi>/`
  - Example ABI directory: `arm64`
- Common files:
  - `base.vdex`: **verified DEX + metadata** (helps avoid repeating bytecode verification; may also contain “quickening” info).
  - `base.odex`: **ELF file** containing **AOT-compiled native code** for methods (ART’s compiled output for the app).
    - Historical naming note: on very old Android builds you might see `.oat`; on modern builds you often see `.odex` + `.vdex` inside an `oat/` directory.
  - `base.art`: an **ART app image** (a snapshot of pre-initialised objects/classes) used to speed up start-up.
    - Correction/nuance: `.art` is mostly “pre-created runtime objects”, not the compiled machine code itself (that’s in `.odex`).
- Relationship to the system/boot images:
  - The platform has its own boot image(s) (`boot*.art`/`boot*.oat`/`boot*.vdex`) holding core framework classes.
  - App artefacts are built **against** that boot image and may reference it; OTAs often trigger regeneration.

Example listing (real-world):
```text
/data/app/.../com.google.android.contacts-.../oat/arm64 # ls -al
-rw-r----- 1 system ...  base.art
-rw-r----- 1 system ...  base.odex
-rw-r----- 1 system ...  base.vdex
```
...
# 1) Find the base APK path
APK=$(adb shell pm path $PKG | head -n 1 | sed 's/^package://')
APPDIR=$(dirname "$APK")

# 2) Try to list oat artefacts (may require root / may be blocked on production builds)
adb shell "ls -al $APPDIR/oat 2>/dev/null"
adb shell "ls -al $APPDIR/oat/arm64 2>/dev/null || ls -al $APPDIR/oat/* 2>/dev/null"

# 3) If you *can* read the files: quick magic checks
adb shell "hexdump -C $APPDIR/oat/arm64/base.odex 2>/dev/null | head"   # should start with ELF magic (7f 45 4c 46)
adb shell "hexdump -C $APPDIR/oat/arm64/base.vdex 2>/dev/null | head"   # often starts with ASCII 'vdex'
adb shell "hexdump -C $APPDIR/oat/arm64/base.art  2>/dev/null | head"   # often starts with ASCII 'art\n'

# 4) Locate system boot images (paths vary)
adb shell 'ls -al /apex/com.android.art/javalib 2>/dev/null | grep -E "boot(\.|-).*\.(art|oat|vdex)" | head'
adb shell 'ls -al /system/framework 2>/dev/null | grep -E "boot(\.|-).*\.(art|oat|vdex)" | head'
```

**PowerShell helper (Windows host):**
```powershell
$pkg = 'com.google.android.contacts'
$apk = (adb shell pm path $pkg | Select-Object -First 1) -replace '^package:',''
$appDir = $apk -replace '/base\.apk$',''
...
**Mini-exercises:**
1) Pick 1 package and answer: do you see `base.odex`/`base.vdex`/`base.art`? Which are missing?
2) Verify file types by magic bytes:
   - `base.odex` starts with `7f 45 4c 46` (ELF)
   - `base.vdex` often starts with `76 64 65 78` (`vdex`)
3) Explain (in your own words) why ART needs both:
   - a **boot image** (system-wide), and
   - per-app compiled artefacts (app-specific).
...
#### Why Zygote makes app start-up fast (and memory efficient)
**In plain terms (simplified):** starting a managed runtime from zero is expensive and mostly the same work for every app. Zygote does that initialisation once (including preloading core classes/resources), then uses `fork()` so new app processes inherit the already-initialised state.

**Notes:**
- Important correction: Zygote still creates **processes** (via `fork()`), not threads.
  - It can *feel* “thread-fast” because `fork()` happens after most of the heavy initialisation work is already done.
- **Copy-on-write (CoW)** is the key optimisation:
  - memory pages that exist before the fork are shared between parent and child until one side writes to a page
  - when a page is written, the kernel copies it so each process gets its own private version
- Zygote listens on Unix domain sockets under `/dev/socket/` (commonly `/dev/socket/zygote`, plus others like `zygote_secondary` and `webview_zygote`) for spawn requests.
- “95% identical start-up”: think “runtime initialisation + core preload is shared”; app-specific class loading and app code execution happen after fork.
- “Double mapped” memory (nuance): it’s common to see the *same file* mapped more than once (or mapped in many processes).
  - You can observe this in `/proc/<pid>/maps` as repeated file paths with different ranges/permissions.
...
# Zygote sockets + permissions/labels
adb shell 'ls -al /dev/socket/zygote* 2>/dev/null'
adb shell 'ls -alZ /dev/socket/zygote* 2>/dev/null || true'

# See zygote socket entries + inodes
adb shell 'cat /proc/net/unix | grep zygote | head -n 20'

# Inspect zygote memory mappings (look for shared boot/runtime artifacts)
adb shell 'ZPID=$(pidof zygote64 2>/dev/null || pidof zygote); echo "zygote pid=$ZPID"; cat /proc/$ZPID/maps | grep -i -E "boot|art|oat|vdex|dex" | head'

# Compare with an app process (replace PID)
# adb shell 'cat /proc/<pid>/maps | grep -i -E "boot|art|oat|vdex|dex" | head'
```

**Mini-exercises:**
1) Find your zygote PID(s) and list the socket path(s) it uses (`/proc/net/unix`).
2) Pick one running app process and compare:
   - `dumpsys meminfo <pid>` for the app vs zygote
   - which mapped files they share (look for the same `boot*.art` / `boot*.oat` entries in `/proc/<pid>/maps`).
3) Explain CoW in one sentence, and why it helps both start-up time and RAM use.
...
### IPC basics (platform-agnostic)
**In plain terms (simplified):** each process has its own virtual memory. That isolation is good for security and stability, but it means processes cannot just “share variables”. **Inter-process communication (IPC)** is how two separate processes exchange data and co-ordinate work.

#### Why IPC exists
- By default, one process cannot read or write another process’s memory (virtual memory is isolated).
- IPC lets processes collaborate safely (e.g., an app asking a system service to do work on its behalf).

#### Common IPC mechanisms
- **Message passing**: send discrete messages between processes.
  - On Android, **Binder** is the main message-passing IPC mechanism.
- **Sockets**: byte streams or datagrams between processes.
  - Unix domain sockets are common for local daemons; network sockets are used for remote communication.
- **Shared memory**: both processes map the same memory region.
  - You still need **synchronisation** (locks/futexes) to avoid races.

#### RPC vs IPC
- **IPC** is the transport.
- **RPC (Remote Procedure Call)** is a programming model that *hides IPC behind a function/method call*.
  - Android examples: AIDL (framework/app), HIDL (older HAL), and AIDL HAL (newer HAL) are “RPC-style” interfaces that typically run over Binder.

#### ADB commands to try
```bash
# Binder devices (IPC driver entry points)
adb shell 'ls -al /dev/*binder*'

# Binder services (userland registry)
adb shell 'service list | head'

# Unix domain sockets (common for daemons)
adb shell 'ls -al /dev/socket | head'
adb shell 'cat /proc/net/unix | head'

# Shared memory hints (device/version-dependent)
adb shell 'ls -al /dev/ashmem 2>/dev/null'
adb shell 'cat /proc/$$/maps | grep -i -E "ashmem|memfd" | head'
```

#### Mini-exercises
1) **Explain the isolation rule**: in one sentence, describe why one process cannot directly read another process’s memory.
2) **Find one Binder service**: run `service list` and pick one service name; guess what it does from its name.
3) **Find one Unix socket**: list `/dev/socket` and pick one socket name; look it up in `init` RC files later (to identify which daemon owns it).

### Android Binder
**In plain terms (simplified):** Binder is Android’s main *local* IPC mechanism. It lets a client process call into a server process (RPC-style), while the kernel driver moves the data and attaches caller identity (UID/PID).
...
#### IPC vs RPC (Binder view)
- Binder is the IPC **transport**.
- AIDL/HIDL/AIDL HAL are RPC-style layers that hide IPC behind a method call.

#### Key concepts
- **Binder driver**: kernel driver that moves transactions between processes (commonly via `/dev/binder`, plus related devices).
- **Service manager**: an “end-point locator” (a phone book). Clients ask it for a service name, and get back a Binder handle.
- **IBinder**: base interface/type for Binder objects/handles.
- **AIDL**: common way to define RPC-style interfaces over Binder.

#### Service managers (end-point locators)
Android uses multiple Binder contexts, each with its own service manager process:

- `servicemanager` + `/dev/binder`: platform/system services (AOSP framework services, `system_server`, etc.).
- `vndservicemanager` + `/dev/vndbinder`: vendor services/daemons (vendor-side Binder services; many vendor AIDL HAL services live here).
- `hwservicemanager` + `/dev/hwbinder`: HIDL HAL services (legacy; still present on many devices).

Example (`ps -ef | grep servicemanager`):
```text
system          603      1 0 09:29:02 ?     00:00:03 servicemanager
system          605      1 0 09:29:02 ?     00:00:02 hwservicemanager
system          606      1 0 09:29:02 ?     00:00:00 vndservicemanager /dev/vndbinder
root          15409  15245 0 14:05:10 pts/1 00:00:00 grep servicemanager
```

Notes:
- The service manager is used to **find** a service; once you have a handle, calls go directly over Binder.
- AIDL-based services can be registered under either `servicemanager` (system) or `vndservicemanager` (vendor) depending on where they live and which Binder device they use.
- If a remote service dies, clients can see `DEAD_OBJECT` (or tools may show “dead”). Reconnect and check logs.

Useful commands:
```bash
# From host (works in PowerShell too)
adb shell 'ps -ef | grep servicemanager'

# Confirm the Binder device nodes exist
adb shell 'ls -al /dev/binder /dev/hwbinder /dev/vndbinder 2>/dev/null'

# List system Binder services (servicemanager)
adb shell 'service list | head'
adb shell 'service check activity'

# HAL inventory (if present)
adb shell 'command -v lshal >/dev/null && lshal | head || echo "lshal not found"'

# Logs
adb shell 'logcat -d | grep -i -E "DEAD_OBJECT|servicemanager|hwservicemanager|vndservicemanager" | tail -n 50'
```

#### Mini-exercises
1) Map the three Binder device nodes to their service managers.
2) Pick one service from `service list` and explain how the service manager acts as an end-point locator.
3) If you have `lshal`, find one HAL service and note whether it is HIDL or AIDL-based.
...
#### Listing services (`service list`)
**In plain terms (simplified):** `service list` asks the (system) Service Manager for its registry and prints the names you can look up.
...
- an index (just for display)
- the **service name** (what you look up)
- the **interface descriptor** in brackets (what the service claims to implement)
...
Notes:
- Names with an instance suffix like `/default` are common for **AIDL HAL** services.
- `service list` talks to the **system** service manager (`servicemanager`, `/dev/binder`).
  - If `vndservice` exists on your build, try `vndservice list` for **vendor** services (`vndservicemanager`, `/dev/vndbinder`).
- If you want to hide names with `/` (so you only see the short service names), you can filter:
  - `service list | grep -v /`

Finding a specific service:
```bash
service list | grep -i statusbar
service list | grep -i phone
service check activity
```

PowerShell helper (host-side filtering):
```powershell
adb shell 'service list' | Select-String -Pattern 'statusbar|phone|activity'
```

#### Calling a service method (`service call`)
**In plain terms (simplified):** `service call` is a low-level tool that sends a Binder transaction by **number**. It’s useful for learning and debugging, but it’s not stable across Android versions (transaction numbers can change).

General pattern:
```bash
service call <serviceName> <transactionCode> [argTypes...]
```

Argument types (common):
- `i32 <n>`: 32-bit integer
- `i64 <n>`: 64-bit integer
- `s16 "text"`: UTF-16 string

Examples (device-dependent):
```bash
# Status bar / notification shade (transaction codes vary by build)
service call statusbar 1
service call statusbar 3

# Example of a typed string argument (use care; often permission-gated)
service call phone 1 s16 "123456"

# Deliberately wrong transaction code (example failure)
service call phone 666
```

Parcel output examples:
```text
...
- `Parcel(00000000 '....')` often means “no error” (success), but it doesn’t guarantee you’ll see a visible UI change.
- If you see `Not a data message`, you likely used the wrong transaction number and/or argument types.
- If a call is blocked, you may see a `SecurityException` in logcat.

Prefer higher-level commands where available (more stable than raw transaction numbers):
```bash
cmd -l | head
cmd statusbar help 2>/dev/null
```
...
### Kernel tracing (ftrace / tracefs) (`/sys/kernel/tracing`)
**In plain terms (simplified):** ftrace is the Linux kernel’s built-in “flight recorder”. You enable a small set of kernel events (tracepoints), trace for a short window, then read the buffer to see what happened.

#### Where it lives
- On many Android devices, tracefs is mounted at:
  - `/sys/kernel/tracing` (common)
  - `/sys/kernel/debug/tracing` (older/alternative)
- Events are organised under:
  - `/sys/kernel/tracing/events/<subsystem>/<event>/`

#### View tracing state (read-only)
```bash
adb shell ls -al /sys/kernel/tracing
adb shell cat /sys/kernel/tracing/tracing_on
adb shell cat /sys/kernel/tracing/current_tracer
adb shell cat /sys/kernel/tracing/available_tracers 2>/dev/null

# Explore tracepoints
adb shell ls /sys/kernel/tracing/events
adb shell ls /sys/kernel/tracing/events/sched
adb shell ls /sys/kernel/tracing/events/binder 2>/dev/null
```

Notes:
- Reading is often allowed for the `shell` user; writing typically needs root (or a userdebug/eng build).
- Keep traces short: enabling lots of events can be noisy and can affect performance.

#### Enable, disable, and view (typical workflow)
1) Stop tracing + clear the buffer
2) Enable the specific event(s) you care about
3) Start tracing, reproduce the behaviour, then stop
4) Read `trace` (snapshot) or `trace_pipe` (live stream)

Device-shell workflow (least confusing for quoting):
```bash
adb shell
...
# Example event: scheduler context switches
echo 1 > events/sched/sched_switch/enable

# Trace for a short window
echo 1 > tracing_on
# reproduce the thing you’re investigating...
echo 0 > tracing_on

# View the trace (can be large)
cat trace
...
Host-side (PowerShell-friendly) viewing:
```powershell
# Snapshot (first 80 lines)
adb shell su -c "cat /sys/kernel/tracing/trace" | Select-Object -First 80

# Live stream (Ctrl+C to stop)
adb shell su -c "cat /sys/kernel/tracing/trace_pipe" | Select-String -Pattern 'sched_switch|binder'
```

Common switches to know:
- Start/stop tracing: `/sys/kernel/tracing/tracing_on` (`1` = on, `0` = off)
- Clear buffer: write an empty string to `/sys/kernel/tracing/trace` (e.g., `: > trace`)
- Enable events:
  - `events/<subsystem>/<event>/enable` (per-event)
  - `events/<subsystem>/enable` (all events in a subsystem)
  - `events/enable` (all events; usually far too noisy)

#### Mini-exercises
1) Explore what’s available:
   - List subsystems under `events/` and pick one (e.g., `sched`, `binder`, `block`, `bpf`).
2) Capture a short scheduler trace:
   - Enable `sched:sched_switch`, trace for 2–5 seconds, then identify a few `prev_comm` / `next_comm` transitions.
3) Compare `trace` vs `trace_pipe`:
   - `trace` is a snapshot; `trace_pipe` streams live.
...
**In plain terms (simplified):** every app has a *package name* (its unique ID). `pm` (PackageManager) can tell you where the APK lives, `adb pull` copies it to your computer, and `aapt2` can read the (compiled) `AndroidManifest.xml` inside it.

#### Step 1: Find a package name
Pick any installed package. If you don’t know one, search by keyword.

PowerShell (Windows host-side filtering):
```powershell
adb shell pm list packages | Select-String -Pattern 'youtube'

# Third‑party apps only (often a shorter list)
adb shell pm list packages -3 | Select-String -Pattern 'google'
```

Bash (macOS/Linux/Git Bash):
```bash
adb shell pm list packages | grep -i youtube
adb shell pm list packages -3 | grep -i google
```

#### Step 2: Ask PackageManager for the APK path
Example (YouTube):
```bash
adb shell pm path com.google.android.youtube
```
Notes:
- Output usually looks like `package:/data/app/.../base.apk`.
- If you see multiple lines, start with the one ending in `base.apk`.
- When copying, drop the `package:` prefix and keep the `/data/app/.../base.apk` part.

#### Step 3: Pull the APK to your host
```bash
adb pull /data/app/.../base.apk youtube.apk
```

#### Step 4: Inspect the manifest on your host
(Requires Android SDK build-tools; see the `aapt/aapt2` notes earlier.)
```bash
...
### Exercise 15: Capture a short kernel trace with ftrace (`/sys/kernel/tracing`) (root likely required)
**In plain terms (simplified):** this records a short “flight recorder” log of kernel scheduler events so you can see processes being switched on/off the CPU.

1) Start a device shell, then become root (if available):
```bash
adb shell
su
```

2) Set up a small trace (scheduler context switches), trace for ~2 seconds, then dump it:
```bash
cd /sys/kernel/tracing
...
# Enable one event (keep it small)
echo 1 > events/sched/sched_switch/enable
...
# View a manageable chunk
cat trace | head -n 80 2>/dev/null || cat trace
...
3) What to look for:
- Lines mentioning `sched_switch` (or similar), often including fields like `prev_comm` and `next_comm`.
- A mix of process names (e.g., `system_server`, `surfaceflinger`, app processes) as the scheduler switches tasks.

Optional:
- If `/sys/kernel/tracing/trace_marker` exists, write your own marker and confirm it appears in the trace:
  - `echo "MARK: about to reproduce" > trace_marker`

### Exercise 16: Explore Binder services with `service list` + `service call` (use care)
**In plain terms (simplified):** this helps you practise the idea that the service manager is a “phone book”, and `service call` is a low-level Binder transaction (RPC-style) by number.

1) Find a couple of interesting services:
```bash
adb shell
service list | head -n 30
service list | grep -i -E 'statusbar|activity|phone'
```

2) Confirm a service exists:
```bash
service check activity
service check statusbar
```

3) Prefer stable `cmd` tooling if available:
```bash
cmd -l | head
cmd statusbar help 2>/dev/null
```

4) Try a low-level call (transaction numbers are device/version-dependent):
```bash
service call statusbar 1
service call statusbar 3
```

5) If something fails, check for exceptions:
```bash
logcat -d | grep -i -E 'SecurityException|statusbar|servicemanager|DEAD_OBJECT' | tail -n 50
```

Takeaways:
- If you see a `Parcel(00000000 '....')`, it often indicates “no error”.
- If you see `Not a data message`, you likely used the wrong transaction code and/or argument types.
- If you see `SecurityException`, you hit a permission/SELinux boundary.
